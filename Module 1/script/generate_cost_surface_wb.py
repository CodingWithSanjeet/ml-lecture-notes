...
def main():
    ...
    w_vals = np.linspace(-1.5, 3.0, 120)
    b_vals = np.linspace(-2.0, 2.0, 120)
    W, B = np.meshgrid(w_vals, b_vals)
    J = np.zeros_like(W, dtype=float)

    for i in range(W.shape[0]):
        for j in range(W.shape[1]):
            J[i, j] = compute_cost_wb(x, y, W[i, j], B[i, j])

    # Find minimum on the grid (should be near w=1, b=0)
    min_idx = np.unravel_index(np.argmin(J), J.shape)
    w_min, b_min, j_min = W[min_idx], B[min_idx], J[min_idx]
    ...
    ax0.set_title("Cost surface J(w,b)")
    ax0.set_xlabel("w")
    ax0.set_ylabel("b")
    ax0.set_zlabel("J(w,b)")
    fig.colorbar(surf, ax=ax0, fraction=0.046, pad=0.04)

    # Right: contour with minimum highlighted
    ax1 = fig.add_subplot(1, 2, 2)
    cs = ax1.contour(W, B, J, levels=25, cmap="viridis")
    ax1.clabel(cs, inline=True, fontsize=7, fmt="%.2f")
    ax1.scatter([w_min], [b_min], color="#2ca02c", s=60, marker="*", zorder=3, label=f"min (w={w_min:.2f}, b={b_min:.2f})")
    ...
    fig.tight_layout()
    out = assets / "cost_surface_wb.png"
